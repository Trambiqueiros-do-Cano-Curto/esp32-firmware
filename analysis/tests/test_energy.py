...
def test_tick_gated_by_period():
    ...
    e.tick(now_ms=1000); assert e.residual == params.INITIAL_BUDGET - params.COST_TICK      # cruzou 1s


def test_residual_floors_at_zero():
    e = energy.EnergyService(energy.calibrated(1, 1, 1))
    e.residual = e.profile.cost_mqtt / 2  # menor que o custo => deve zerar (piso)
    e.on_mqtt_publish()
...
def test_peer_ttl_expires():
    ...
    assert e.get_peer_residual(b"\x00\x00\x00\x00\x00\x02", now_ms=params.PEER_TTL_MS) == (0.0, False)
